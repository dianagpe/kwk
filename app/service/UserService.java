...
public class UserService extends BaseUserService {
    private HashMap<String, IdentityUser> users  = new HashMap<String, IdentityUser>();
    ...
    public Identity doSave(Identity identity) {
        IdentityUser user = IdentityUser.save(identity);

        if(user != null)
            Cache.set(user.identityId.providerId()+"-"+user.identityId.userId(), user, 60*30);
        return user;
    }
    ...
    public Identity doFind(IdentityId userId) {
       // return IdentityUser.find(userId);
       return (IdentityUser) Cache.get(userId.providerId()+"-"+userId.userId());
